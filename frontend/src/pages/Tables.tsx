...
export default function Tables() {
  ...
  if (user?.role !== 'admin' && user?.role !== 'staff') {
    return (
      <Box className="p-5">
        <Typography variant="h4" className="page-heading mb-2">
          Tables
        </Typography>
  ...
  const busyOrderForTable = (n: number) =>
    orders.find((o) => o.tableNumber === n && ['pending', 'preparing'].includes(o.status));
  const busyResForTable = (n: number) =>
    reservations.find((r) => r.tableNumber === n && r.status === 'confirmed');
