  ...
  return (
    <Box>
      ...
      <Typography variant="body1" className="mb-1">
        Welcome, <strong>{user?.name}</strong>! {todaySummary && <span className="text-gray-500">— {todaySummary}</span>}
      </Typography>

      {/* Primary stats */}
      <Box className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        <DashboardStatCard title="Total Orders" value={stats.totalOrders} color="#e94560" subtitle={`${stats.pendingOrders} pending • ${stats.completedOrders} completed`} />
        <DashboardStatCard title="Reservations" value={stats.totalReservations} color="#0f3460" subtitle={`${stats.confirmedReservations} confirmed`} />
        <DashboardStatCard title="Menu Items" value={stats.totalMenuItems} color="#16a085" subtitle={`${stats.availableMenuItems} available • ${stats.totalCategories} categories`} />
        <DashboardStatCard title="Revenue" value={`$${stats.totalRevenue.toFixed(2)}`} color="#6c5ce7" subtitle="Completed orders" />
      </Box>

      {/* Secondary stats */}
      <Box className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
        <DashboardStatCard title="Tables" value={`${stats.busyTables}/${stats.totalTables}`} color="#f39c12" subtitle={`${stats.freeTables} free`} />
        ...
        <DashboardStatCard title="Today Reservations" value={stats.todayReservations} color="#a29bfe" />
      </Box>
          ...
          <Box className="flex flex-wrap gap-2">
            {stats.ordersByStatus.length === 0 ? (
              ...
              stats.ordersByStatus.map((s) => (
                <Box key={s.status} className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-md border">
        ...
        {stats.recentOrders.length === 0 ? (
          ...
          <Box className="flex flex-col gap-2">
            {stats.recentOrders.map((o) => (
              <Box key={o._id} className="flex justify-between items-center p-3 bg-gray-50 rounded-md border">
                <Box>
                  <strong className="text-sm">Order #{o._id.slice(-6).toUpperCase()}</strong>
                  {o.tableNumber && <span className="text-sm text-gray-500"> • Table {o.tableNumber}</span>}
                  <p className="text-xs text-gray-500">{new Date(o.createdAt).toLocaleString()} {o.user?.name ? `• ${o.user.name}` : ''}</p>
                </Box>
