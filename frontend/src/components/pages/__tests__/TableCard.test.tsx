...
describe('TableCard', () => {
  ...
  it('shows busy with order', () => {
    render(
      <TableCard
        table={{ number: 1, isBusy: true, busyType: 'order', occupiedBy: 'o1' }}
        order={{ _id: 'order123456', status: 'pending' }}
      />,
    );
  ...
  it('shows reservation details', () => {
    render(
      <TableCard
        table={{ number: 2, isBusy: true, busyType: 'reservation', occupiedBy: 'r1' }}
        reservation={{ guests: 4, time: '19:00' }}
      />,
    );
  ...
  it('applies busy/free classes', () => {
    const { container: busy } = render(
      <TableCard table={{ number: 1, isBusy: true, busyType: 'order', occupiedBy: 'o1' }} />,
    );
    expect(busy.firstChild).toHaveClass('table-card-busy');
    const { container: free } = render(
      <TableCard table={{ number: 2, isBusy: false, busyType: null, occupiedBy: null }} />,
    );
