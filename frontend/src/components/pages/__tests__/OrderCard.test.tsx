...
describe('OrderCard', () => {
  it('renders order id and table', () => {
    render(<OrderCard order={mockOrder()} isStaffView isOwner={false} onEdit={jest.fn()} onUpdateStatus={jest.fn()} onDelete={jest.fn()} />);
  ...
  it('hides user info for customer view', () => {
    render(<OrderCard order={mockOrder()} isStaffView={false} isOwner={false} onEdit={jest.fn()} onUpdateStatus={jest.fn()} onDelete={jest.fn()} />);
    expect(screen.queryByText(/By: John/)).not.toBeInTheDocument();
  });

  it('shows Edit and Start Preparing for pending', () => {
    render(<OrderCard order={mockOrder({ status: 'pending' })} isStaffView isOwner onEdit={jest.fn()} onUpdateStatus={jest.fn()} onDelete={jest.fn()} />);
  ...
  it('calls onEdit when Edit clicked', () => {
    ...
    render(<OrderCard order={mockOrder()} isStaffView isOwner onEdit={onEdit} onUpdateStatus={jest.fn()} onDelete={jest.fn()} />);
  ...
  it('calls onUpdateStatus for preparing actions', () => {
    ...
    render(<OrderCard order={mockOrder({ status: 'preparing' })} isStaffView isOwner onEdit={jest.fn()} onUpdateStatus={onUpdateStatus} onDelete={jest.fn()} />);
  ...
  it('shows Delete for completed', () => {
    ...
    render(<OrderCard order={mockOrder({ status: 'completed' })} isStaffView isOwner onEdit={jest.fn()} onUpdateStatus={jest.fn()} onDelete={onDelete} />);
