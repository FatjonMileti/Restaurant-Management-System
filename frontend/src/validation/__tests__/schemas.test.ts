...
describe('frontend validation schemas', () => {
  ...
  describe('registerSchema', () => {
    it('rejects short password', () => {
      expect(
        registerSchema.safeParse({ name: 'John', email: 'john@example.com', password: '123' })
          .success,
      ).toBe(false);
    });
  });
  ...
  describe('menuItemSchema', () => {
    ...
    it('rejects negative price', () => {
      expect(menuItemSchema.safeParse({ name: 'Pizza', price: -5, category: 'Food' }).success).toBe(
        false,
      );
    });
  });
  ...
  describe('restaurantSettingsSchema', () => {
    ...
    it('requires tableCount >=1', () => {
      expect(restaurantSettingsSchema.safeParse({ name: 'Rest', tableCount: 0 }).success).toBe(
        false,
      );
    });
  });
